...
def is_possible_game(game):
    for subset in game:
        ...
        for color_count in colors:
            count, color = color_count.split()  # Adjusted the order here
        ...
        if (
            counts["red"] > red_count
            or counts["green"] > green_count
            or counts["blue"] > blue_count
        ):
            return False
...
def sum_of_possible_game_ids(input_str):
...
result = sum_of_possible_game_ids(games_input)
